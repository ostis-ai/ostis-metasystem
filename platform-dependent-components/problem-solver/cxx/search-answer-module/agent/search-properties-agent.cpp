...
ScResult SearchPropertiesAgent::DoProgram(ScActionInitiatedEvent const & event, ScAction & action)
{
  ...
  answerTemplate.Quintuple(
        ...
        ScType::EdgeAccessVarPosPerm >> DerivalConstants::RREL_EDGE,
        ScType::NodeVar >> SearchAnswerConstants::SET_NODE);

  ScAddrVector propertiesVector = findProperties(conceptAddr);
  ...
  while (j < i)
  {
    std::string EDGE_ALIAS = DerivalConstants::VAR_EDGE + std::to_string(j);
...
ScAddrVector SearchPropertiesAgent::findProperties(ScAddr const & conceptAddr)
{

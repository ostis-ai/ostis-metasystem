...
{
  auto [linkAddr, formulaAddr] = action.GetArguments<2>();
  ...
  action.SetArguments(processingProgramAddr, generateNonAtomicActionArgsSet(messageAddr));
...
ScAddr MessageReplyAgent::generateAnswer(ScAddr const & messageAddr)
{
  ...
  ScIterator3Ptr classesIt = m_context.Iterator3(ScType::NodeConstClass, ScType::EdgeAccessConstPosPerm, messageAddr);
...
bool MessageReplyAgent::linkIsValid(ScAddr const & linkAddr)
{
  ...
  ScIterator3Ptr const conceptTextFileIterator = m_context.Iterator3(

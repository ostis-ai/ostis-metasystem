...
namespace verificationModule
{
  ...
  {
    std::string description = "Check results for " + checkResult.setIdtf + ". Check performed at "
                              + checkResult.checkTime + ".\n Checked set belong to "
                              + checkResult.subjectDomainContainingAsMaximumClass + " as maximum studed object class";
    ...
    if (!checkResult.subjectDomainsContainingAsNotMaximumClass.empty())
    {
      ...
      description += "and to " + subjectDomainsContainingAsNotMaximumClassEnumeration
                     + " as not maximum studied object classes.\n";
    }
    else
      description += ".\n";
